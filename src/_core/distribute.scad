...
function segmentsCrossing( 
	...
	assert( in_list(dir,[ HORIZONTAL,VERTICAL ])							,"[segmentsCrossing] Wrong [dir] parameter")
	assert( spacing															,"[segmentsCrossing] Missing [spacing] argument")
	assert( dir == HORIZONTAL 	? in_list(origin,[CENTER,BACK,FWD])   : true,"[segmentsCrossing] When [dir] is HORIZONTAL then [origin] should be CENTER,BACK or FWD" )
	assert( dir == VERTICAL 	? in_list(origin,[CENTER,LEFT,RIGHT]) : true,"[segmentsCrossing] When [dir] is VERTICAL then [origin] should be CENTER,LEFT or RIGHT" )
	let (
		// Polygon dimensions	
