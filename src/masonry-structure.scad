...
//
// Example(3D,Huge): Automatic sizing slab with parent space
//   space(3,3,2,debug=true,except=[FRONT,RIGHT])
//      slab($color=primary());
// Log: Provide information for slab with unit_price of 125$ per m3;
//   slab(l=3,w=3,thickness=200,info=true,unit_price=125);
module slab( 
...
// Example(3D,Big): Simple wall footing
//   wallFooting(l=5, w=0.6, thickness=300);    
// Example(3D,Big): Wall footing for space 
//   space(4,4,0.4,debug=true)
//      wallFooting(w=0.6, thickness=300,$color=primary());   
// Log : Provide wall footing info (info=true) for preceding example
//   space(4,4,0.4) wallFooting(w=0.6, thickness=300,info=true);   
module wallFooting(
	l,
	w,
    thickness 	= 300,
...
// Side Effects:
//   $meta = Stores metadata (name, volume, weight, unit_price, cost, IFC properties) if info=true.
// Example(3D,Big): Footing pad with top pillar
//   include <wood-structure.scad>
//   primary() footingPad(l=0.6, w=0.6, thickness=300 )
//      attach(TOP)
//         rectPillar(l=2,section=[200,200],material="Concrete",anchor=BOT);
// Log: Information for footing pad 
//   footingPad (l=0.6, w=0.6, thickness=300, info=true );
module footingPad(
    l,
    w,
    thickness,
...
//   $meta = Stores metadata (name, volume, weight, unit_price, cost, IFC properties) if info=true.
// Example(3D,Big):
//   primary() footingPileCap(l=3, w=3, thickness=600) 
//      attach(	BOT, TOP,align=corners(TOP)	) pile(depth=4);
// Log: Info for next example with unit price of 120$/m³
//   footingPileCap(l=3, w=3, thickness=600, material="Concrete", unit_price=120,info=true);
module footingPileCap(
    ...
    assert(is_meters(l), 					"[footingPileCap] l is undefined. Provide length or define $space_length");
    assert(is_meters(w), 					"[footingPileCap] w is undefined. Provide width or define $space_width");
    assert(is_num_positive(thickness) , 	"[footingPileCap] thickness must be positive");
    ...
    attachable( anchor=anchor, spin=spin, orient=orient, size=size) {
		material(material) cuboid(size,rounding=20);
        children();
    }
}
...
//   depth 		= Caisson depth in meters (default: 3).
//   material 	= Material name from masonry.scad (default: "Concrete").
//   unit_price	= Cost per cubic meter in currency units (default: 100).
...
//   $meta = Stores metadata (name, volume, weight, unit_price, cost, IFC properties) if info=true.
// Example(3D,Big):
//   footingCaisson(diam=1.2, depth=4, unit_price=120);
// Log: Info for previous example
//   footingCaisson(diam=1.2, depth=4, unit_price=120, info=true);
module footingCaisson(
    diam 		= 1,
    depth 		= 3,
    material 	= "Concrete",
    ...
    assert(is_meters(depth), 						"[footingCaisson] depth must be positive");
...
//   depth 		= Pile depth in meters (default: 10).
...
//   $meta = Stores metadata (name, volume, weight, unit_price, cost, IFC properties) if info=true.
// Example(3D,Big):
//   pile(diam=0.4, depth=12, type="BORED", unit_price=120);
// Log: Info for previous example
//   pile(diam=0.4, depth=12, type="BORED", unit_price=120, info=true);   
module pile(
    diam 		= 0.3,
    depth 		= 10,
    type 		= "DRIVEN",
    ...
    assert(is_meters(diam), 	"[pile] diam must be positive");
    assert(is_meters(depth), 			"[pile] depth must be positive");
